...
# ========================= 
# NSE FETCHER - ROBUST ZIP HANDLING
# ========================= 
def fetch_nse_bhavcopy_smart(trade_date, date_str):
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            # Try ZIP with multiple strategies
            for skip_bytes in [0, 4, 8, 16]:
                try:
                    test_content = content[skip_bytes:] if skip_bytes > 0 else content
                    z = zipfile.ZipFile(io.BytesIO(test_content))
                    if skip_bytes > 0:
                        logger.info(f"✅ ZIP found by skipping {skip_bytes} bytes")
                    else:
                        logger.info(f"✅ Direct ZIP file")
                    logger.info(f"   Files in ZIP: {z.namelist()}")
                    return z
                except zipfile.BadZipFile:
                    continue
                except Exception as e:
                    logger.warning(f"   Skip {skip_bytes} bytes failed: {e}")
                    continue
            
            # If ZIP attempts failed, try JSON
            try:
                text_content = content.decode('utf-8')
                data = json.loads(text_content)
                logger.info(f"   JSON response: {type(data)}")
                ...
                logger.warning(f"⚠️ Not valid JSON either")
            
            logger.error(f"❌ Could not parse NSE response as ZIP or JSON")
...
def process_nse_data(z):
    try:
        ...
        logger.info(f"📊 NSE Total rows: {len(df_nse)}")
